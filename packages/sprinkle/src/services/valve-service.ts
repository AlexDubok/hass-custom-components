...
export class ValveService {
  ...
  constructor(haService: HomeAssistantService, config: SprinkleConfig) {
    ...
    this.timedIrrigation = this.haService.getEntityState(
      config.timed_irrigation_entity
    );
    this.quantitativeIrrigation = this.haService.getEntityState(
      config.quantitative_irrigation_entity
    );
  }
